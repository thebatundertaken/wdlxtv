...
public class DummyControllerActivity extends AppCompatActivity {
    ...
    private void getPreferences() {
        ...
        if (!this.conf_shakecontrol && this.MyShake != null) {
            MyShake.close();
            MyShake = null;
        } else if (this.conf_shakecontrol && this.MyShake == null) {
            MyShake = new ShakeListener((SensorManager) getSystemService(Context.SENSOR_SERVICE));
            MyShake.setForceThreshHold(1.9d);
            MyShake.setOnShakeListener(() -> {
                if (lastshake != null) {
